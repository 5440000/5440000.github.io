...
const $ = {
  modal(options) {                                 //обьявляю метод modal для обьекта доллар и передаю ему options
  console.log(options)
...
arrButtons.forEach((button) => {
  button.addEventListener("click", (e) => {
    const goodId = e.target.dataset.id;         //определяю айди на который кликнули
    console.log(e.target.dataset.id)
    const modal = $.modal(arrItems[goodId]);//задаю переменной modal значение айди на который кликнули и передаю его в метод modal оьекта доллар
    modal.open();
  });
});
console.log
