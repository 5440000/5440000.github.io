const _createModal = ({ img, name, price, currancy, madein }) => {
  ...
  modalClose.addEventListener("click", () => {
      ...
      // goodMadeIn.insertAdjacentHTML("afterbegin",`Наручные часы <span id="good_title"></span>`)
      // price = ""
      // currancy = ""
      // madein = ""
    ...
    modal.removeChild(goodName)
    console.log("🚀 ~ file: goodModal.js ~ line 32 ~ modalClose.addEventListener ~ parentGoodName", parentGoodName)
    const goodNameNew = document.createElement('div')
    goodNameNew.classList.add("#good_title")
    goodNameNew.insertAdjacentHTML("afterbegin",`
    <span id="good_title"></span
            ></span>
    `)
