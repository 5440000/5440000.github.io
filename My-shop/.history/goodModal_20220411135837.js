...
const refreshModalData = (goodName,goodMark) => {
  (goodName) =>{
  const parentGoodName = goodName.parentNode;
  goodName.parentNode.removeChild(goodName);
  const goodSpanNew = document.createElement("span");
  goodSpanNew.id = "good_title";
  parentGoodName.append(goodNameNew)};

  (goodMark) =>{
    const parentGoodMark = goodMark.parentNode;
    goodMark.parentNode.removeChild(goodMark);
    const goodSpanNew = document.createElement("span");
    goodSpanNew.id = "good_title";
    parentGoodMark.append(goodMarkNew)};
  }
};
