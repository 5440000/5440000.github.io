(function () {
  ...
  const createArticle = (articlePreview) => {
  ...
  const getData = async () => {
    ...
    DATA_FROM_SERVER = await json.json();
    renderPage();
  ...
  const createStartQuerySting = () => {
    history.replaceState(
      DATA_FOR_RENDER,
      "",
      `?year=${DATA_FOR_RENDER.year}&keyword=${DATA_FOR_RENDER.keyword}&pageindex=${DATA_FOR_RENDER.pageindex}`
    );
  };

  const checkQueryString = () => {
    return HREF.search === "" ? false : true;
  };

  const getQueryStringValues = () => {
    DATA_FOR_RENDER.year = HREF.searchParams.get("year");
    DATA_FOR_RENDER.keyword = HREF.searchParams.get("keyword");
    DATA_FOR_RENDER.pageindex = HREF.searchParams.get("pageindex");
  };
  const renderArticles = () => {
    const firstindex =
      DATA_FOR_RENDER.pageindex * ITEMS_ON_PAGE - ITEMS_ON_PAGE;
    const lastindex = DATA_FOR_RENDER.pageindex * ITEMS_ON_PAGE;

    if (DATA_FOR_RENDER.year === "all-year") {
      DATA_FROM_SERVER.slice(firstindex, lastindex).map(createArticle);
    }
    if (DATA_FOR_RENDER.year !== "all-year") {
      DATA_FROM_SERVER.filter((article) => article.year == DATA_FOR_RENDER.year)
  ...
  const getSearchValue = () => {
    ...
    searchInput.addEventListener("change", (event)=>{
      ...
      DATA_FOR_RENDER.keyword = event.target.value});
      console.log(DATA_FOR_RENDER.keyword);  
  ...
  const renderPage = () => {
    ...
    if (checkQueryString() === false) {
      ...
      createStartQuerySting();
      renderArticles();
    } else {
      console.log("not first loading");
